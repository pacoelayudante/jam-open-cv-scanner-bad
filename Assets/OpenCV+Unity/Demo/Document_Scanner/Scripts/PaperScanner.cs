...
public class PaperScanner
{
    ...
    private void CalculateOutput()
    {
        ...
        var dest = matEdges.Clone().CvtColor(ColorConversionCodes.GRAY2BGR);
        // foreach (var contour in goodCandidates)
        {
            Moments m = Cv2.Moments(hull);
            int cx = (int)(m.M10 / m.M00);
            int cy = (int)(m.M01 / m.M00);

             Cv2.DrawContours(dest, new Point[][] {hull}, 0, Scalar.FromRgb(0,255 ,255), 3);
        }
        display = dest.Resize(new Size(matInput_.Width, matInput_.Height));
